...
export const useSetDefaultScale = ({
  ...
  useEffect(() => {
    const getDefaultScale = () => {
      ...
      const resumeControlBarHeightRem = parseFloat(
        CSS_VARIABLES["--resume-control-bar-height"]
      );
      const resumePaddingRem = parseFloat(CSS_VARIABLES["--resume-padding"]);
      const overheadPx =
        (topNavBarHeightRem + resumeControlBarHeightRem + resumePaddingRem * 2) *
        PX_PER_REM;
      const resumeHeightPx = screenHeightPx - overheadPx;
